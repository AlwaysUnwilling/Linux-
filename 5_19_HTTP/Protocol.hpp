  ...
  private:
    bool RcvHttpRequestLine()
    {
      if(Util::ReadLine(sock,http_request.request_line)>0){
        http_request.request_line.resize(http_request.request_line.size()-1);  //如果读取时出错可能会导致resize时失败服务器会直接崩溃
      }else{
        stop=true;
      }
      return stop;
  ...
  public:
    ...
    void RcvHttpRequest()
    {
      if(RcvHttpRequestLine() || RcvHttpRequestHeader()){
        //读取出错
        LOG(ERROR,"Rcv ERROR!");
      }else{      //上面俩条件都不成立--即stop=false,读取成功
...
class CallBack
{
  public:
    /*static void* HandlerRequest(void* arg)
    {
      ...
      ep->RcvHttpRequest();
      if(!ep->IsStop()){
        LOG(INFO,"Rcv Success! Continue Build And Send!");
        ep->BuildHttpResponse();
        ep->SendHttpResponse();
      }else{
        LOG(WARNING,"Rcv Fail!Stop Build And Send!");
      }
      delete ep;
